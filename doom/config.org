...
(defvar jecs/default-frame-cols 220)
(defvar jecs/default-frame-rows 70)
...
(defun jecs/apply-frame-size-smart (&optional frame)
  (let* ((frame (or frame (selected-frame)))
    ...
    ;; First, try the normal/default size.
    (set-frame-parameter frame 'fullscreen nil)
    (set-frame-position frame wa-left wa-top)
    (set-frame-size frame
                    jecs/default-frame-cols
                    jecs/default-frame-rows)

    ;; Now measure the real frame size, not an estimate.
    (let ((frame-width  (frame-outer-width frame))
          (frame-height (frame-outer-height frame)))
      (when (or (> frame-width wa-width)
                (> frame-height wa-height))
        ;; Doesn't fit: maximize it.
...
(defun jecs/disable-remote-vc-h ()
  (when (file-remote-p default-directory)
    (setq-local vc-handled-backends nil)))

(add-hook 'find-file-hook #'jecs/disable-remote-vc-h)
#+end_src
