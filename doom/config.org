...
(after! multiple-cursors
  ;; Never ask; run unknown commands for every cursor.
...
Janet is a small embeddable lisp language.

#+begin_src emacs-lisp :results none
(after! lsp-mode
  (dolist (mode '(janet-mode janet-ts-mode))
    (add-to-list 'lsp-language-id-configuration
                 (cons mode "janet")))
  ...
  (lsp-register-client
   (make-lsp-client
    ...
    (lsp-stdio-connection
     '("janet-lsp")
     (lambda () (executable-find "janet-lsp")))
    ...
    (lambda (_client callback error-callback _update?)
      (lsp-async-start-process
       ...
       "jpm" "install"
       "https://github.com/CFiggers/janet-lsp.git")))))

(add-hook! '(janet-mode-hook janet-ts-mode-hook)
           #'lsp-deferred)

(after! projectile
  (add-to-list 'projectile-project-root-files "project.janet"))

(defvar-local jecs/janet-buildpath "build/"
  "JPM build path used by the Janet REPL.")

(defvar-local jecs/janet--evaluation nil)
(defvar-local jecs/janet--protocol-carry "")

(defun jecs/janet--marker-prefix-length (text marker)
  (let ((prefix-length
         (min (1- (length marker))
              (length text))))
    ...
    (while
        (and (> prefix-length 0)
             (not
              (string-suffix-p
               (substring marker 0 prefix-length)
               text)))
      (setq prefix-length (1- prefix-length)))

    prefix-length))

(defun jecs/janet--frame-chunk (text marker)
  (if-let ((start
            (string-match
             (regexp-quote marker)
             text)))
    ...
    (let* ((held-length
            (jecs/janet--marker-prefix-length
             text marker))
           (split (- (length text) held-length)))

      (list nil
            (substring text 0 split)
            (substring text split)))))

(defun jecs/janet--filter-framed-output (text)
  (if (not jecs/janet--evaluation)
      (prog1
          (concat jecs/janet--protocol-carry text)
        (setq jecs/janet--protocol-carry ""))

    (let ((phase
           (plist-get jecs/janet--evaluation :phase)))

      (pcase phase
        (:waiting-start
         (let* ((start-marker
                 (plist-get
                  jecs/janet--evaluation
                  :start-marker))
                (data
                 (concat
                  jecs/janet--protocol-carry
                  text)))

           (pcase-let
               ((`(,found ,_before ,after)
                 (jecs/janet--frame-chunk
                  data start-marker)))
             ...
             (if found
                 (progn
                   ...
                   (jecs/janet--filter-framed-output
                    after))

               (setq jecs/janet--protocol-carry after)
               ""))))
...
(defun jecs/janet--discard-evaluation (repl)
  (when (buffer-live-p repl)
    (with-current-buffer repl
      (when-let ((state jecs/janet--evaluation))
        (let ((position (plist-get state :position))
              (output (plist-get state :output))
              (process (get-buffer-process repl)))

          (setq jecs/janet--evaluation nil)
          (setq jecs/janet--protocol-carry "")

          (remove-hook
           'comint-redirect-hook
           #'jecs/janet--schedule-evaluation-finished
           t)
...
(defun jecs/janet--repl-arguments (root buildpath)
  (let ((arguments nil)
        (local-tree
         (expand-file-name "jpm_tree" root))
        (native-path
         (expand-file-name buildpath root)))

    (when (file-directory-p local-tree)
      (setq arguments
            (append arguments '("--local"))))

    (append
     arguments
     (list
      (format "--buildpath=%s" buildpath)
      "janet"
      "-e"
      (format
       "(module/add-syspath %S)"
       native-path)
      "-s"))))

(defun jecs/janet-repl ()
  (interactive)
  (require 'comint)
  ...
  (let* ((root
         ...
         (buildpath jecs/janet-buildpath)
         (arguments
          (jecs/janet--repl-arguments
           root buildpath))
         (name
          (format
           "janet:%s"
           (doom-project-name root)))
         (buffer
          (get-buffer-create
           (format "*%s*" name))))

    (unless (comint-check-proc buffer)
      (jecs/janet--discard-evaluation buffer)
...
(defun jecs/janet--register-repl (source repl)
  (with-current-buffer source
    (let* ((mode major-mode)
      ...
      (unless root
        (user-error "Janet REPL requires a project"))
...
(defun jecs/janet--at-prompt-p ()
  (save-excursion
    (goto-char (point-max))
    (forward-line 0)
    (looking-at-p comint-prompt-regexp)))

(defun jecs/janet--ensure-repl ()
  (let ((source (current-buffer))
        (repl (+eval-current-repl-buffer)))

    (unless (and repl
                 (comint-check-proc repl))
      (setq repl (jecs/janet-repl))
      (jecs/janet--register-repl source repl))

    (with-current-buffer repl
      (unless jecs/janet--evaluation
        (let ((process
          ...
          (while (and process
                      ...
                      (< (float-time) deadline)
                      (not
                       (jecs/janet--at-prompt-p)))
            (accept-process-output process 0.05))

          (unless (jecs/janet--at-prompt-p)
            (user-error
             "Janet did not produce a REPL prompt")))))

    repl))

(defun jecs/janet--extract-result (text state)
  (let* ((start-marker
          (plist-get state :start-marker))
         (result-marker
          (plist-get state :result-marker))
         (start-position
          (string-match
...
(defun jecs/janet--schedule-evaluation-finished ()
  (let ((repl (current-buffer)))
    (run-at-time
     0 nil
     #'jecs/janet--evaluation-finished
     repl)))

(defun jecs/janet--evaluation-finished (repl)
  (when (buffer-live-p repl)
    (with-current-buffer repl
      (when-let ((state jecs/janet--evaluation))
        (let* ((source
...
(defun jecs/janet--mirror-input (code)
  (goto-char (point-max))
...
(defun jecs/janet-eval-region (beg end)
  (interactive "r")
  (require 'json)

  (let* ((source (current-buffer))
         (position (copy-marker end))
         (code
          (buffer-substring-no-properties
           beg end))
         (repl
          (jecs/janet--ensure-repl))
         (output
          (generate-new-buffer
           " *janet-eval*"))
         (id
         ...
         (start-marker
          (format
           "\x1eJANET_START_%s\x1f"
           id))
         (result-marker
          (format
           "\x1eJANET_RESULT_%s\x1f"
           id))
         (command
          (format
           (concat
            ...
            "(file/write stdout %s) "
            "(file/flush stdout) "
            "(let [result (eval-string %s)] "
            "(file/write stdout %s) "
            "(file/flush stdout) "
            "result))")
           (json-encode-string start-marker)
           (json-encode-string code)
           (json-encode-string result-marker))))

    (with-current-buffer repl
      (when jecs/janet--evaluation
        (kill-buffer output)
        (set-marker position nil)

        (user-error
         "A Janet evaluation is already running"))

      (jecs/janet--mirror-input code)

      (setq jecs/janet--protocol-carry "")

      (setq jecs/janet--evaluation
            (list
             :source source
             :position position
             :output output
      ...
      (condition-case error-data
          (comint-redirect-send-command-to-process
           command output repl t t)

        (error
         (jecs/janet--discard-evaluation repl)

         (signal
          (car error-data)
          (cdr error-data)))))))

(defun jecs/janet-eval-last-sexp ()
  (interactive)
  ...
  (save-excursion
    ...
    (let ((end (point)))
      ...
      (jecs/janet-eval-region
       (point)
       end))))

(defun jecs/janet-eval-defun ()
  (interactive)
...
(defun jecs/janet-interrupt-eval ()
  (interactive)

  (let ((repl
         (+eval-current-repl-buffer)))

    (unless (and repl
                 (comint-check-proc repl))
      (user-error
       "There is no running Janet REPL"))

    (with-current-buffer repl
      (unless jecs/janet--evaluation
        (user-error
         "No Janet evaluation is running"))
    ...
    (message
     "Janet evaluation aborted; the REPL will restart on the next evaluation")))

(set-repl-handler! '(janet-mode janet-ts-mode)
  #'jecs/janet-repl
  :persist t
  :send-region #'jecs/janet-eval-region
  :send-buffer
  (lambda ()
    (jecs/janet-eval-region
     (point-min)
...
(map! :after janet-mode
      :map janet-mode-map
      "C-x C-e" #'jecs/janet-eval-last-sexp
      "C-M-x"   #'jecs/janet-eval-defun
      "C-c C-r" #'jecs/janet-eval-region
      "C-c C-k" #'jecs/janet-interrupt-eval)
