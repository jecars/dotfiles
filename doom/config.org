...
(setq +dashboard-banner-vertical-padding '(0 . 0))
...
(defun jecs/dashboard-buffer-setup-h ()
  ;; actually kill the mode line
  (setq-local mode-line-format nil)
  ;; optional: also change mode name if mode line ever reappears
  (setq-local mode-name "")
  ;; avoid the highlight stripe
  (setq-local hl-line-mode nil)
  (when (fboundp 'hide-mode-line-mode)
    (hide-mode-line-mode 1))
  (force-mode-line-update))

(defun jecs/dashboard-hide-modeline-h ()
  (when (+dashboard-buffer-p (current-buffer))
    (setq-local mode-line-format nil)
    (force-mode-line-update)))

(add-hook 'doom-switch-buffer-hook #'jecs/dashboard-hide-modeline-h) ; inhibit modeline on initial load
(add-hook 'window-configuration-change-hook #'jecs/dashboard-hide-modeline-h) ; inhibit modeline on dashboard mode trigger
